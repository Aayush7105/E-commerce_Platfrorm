...
function WishlistToggleButton({ product }) {
  ...
  return (
    <button
      ...
      className={`motion-icon-button absolute left-4 top-4 z-10 inline-flex h-9 w-9 items-center justify-center rounded-full border ${
        isWishlisted
          ? 'border-red-400/70 bg-red-500/25 text-red-300 hover:bg-red-500/35'
          : 'border-white/25 bg-black/35 text-white hover:bg-black/60'
      }`}
    >
      <FiHeart className={`h-4 w-4 ${isWishlisted ? 'motion-wishlist-heart fill-current' : ''}`} />
    </button>
  )
}
